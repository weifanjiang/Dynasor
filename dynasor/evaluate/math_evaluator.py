...
class MathEvaluator:
    def __init__(self, voting: str = 'majority'):
        if voting == 'majority':
            self.voting = majority_vote
        else:
            raise ValueError(f"Invalid voting method: {voting}")

    def evaluate(self, results: list[dict], ground_truths: list[dict], extract_func: Callable = None):
        correct = 0
        for result, ground_truth in zip(results, ground_truths):
            if extract_func:
                result = [extract_func(res, 'gsm8k') for res in result]
            print(result, ground_truth)
            if math_equal(self.voting(result), ground_truth):
                correct += 1
